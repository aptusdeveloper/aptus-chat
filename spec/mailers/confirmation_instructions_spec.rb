...
RSpec.describe 'Devise::Mailer' do
  describe 'notify' do
    ...
    it 'has the correct header data' do
      expect(mail.reply_to).to contain_exactly('no-reply@aptuschat.local')
    ...
    it 'shows the default confirmation state' do
      ...
      expect(mail_body).to include('Só precisamos verificar seu endereço de e-mail antes de começar a usar sua conta.')
      expect(mail_body).to include('Confirmar minha conta')
      expect(mail_body).not_to include('Convite para workspace')
    end
    ...
    context 'when there is an inviter' do
      ...
      it 'refers to the inviter and their account' do
        ...
        expect(mail_body).to include("#{inviter_val.name} convidou você para ingressar no workspace #{account.name}")
