...
RSpec.describe AgentNotifications::ConversationNotificationsMailer do
  ...
  describe 'conversation_creation' do
    ...
    it 'renders the subject' do
      expect(mail.subject).to eq("#{agent.available_name}, uma nova conversa [ID - #{conversation
        .display_id}] foi criada em #{conversation.inbox&.sanitized_name}.")
    end
  ...
  describe 'conversation_assignment' do
    ...
    it 'renders the subject' do
      expect(mail.subject).to eq("#{agent.available_name}, uma nova conversa [ID - #{conversation.display_id}] foi atribuída a você.")
    end
  ...
  describe 'conversation_mention' do
    ...
    it 'renders the subject' do
      expect(mail.subject).to eq("#{agent.available_name}, você foi mencionado na conversa [ID - #{conversation.display_id}]")
    end
    ...
    it 'renders the senders name' do
      expect(mail.body.encoded).to match("Você foi mencionado em uma conversa. <b>#{another_agent.display_name}</b> escreveu:")
    end
  ...
  describe 'assigned_conversation_new_message' do
    ...
    it 'renders the subject' do
      expect(mail.subject).to eq("#{agent.available_name}, nova mensagem na conversa atribuída a você [ID - #{message.conversation.display_id}].")
    end
  ...
  describe 'participating_conversation_new_message' do
    ...
    it 'renders the subject' do
      expected_subject = "#{agent.available_name}, nova mensagem na conversa da qual você participa " \
                         "[ID - #{message.conversation.display_id}]."
