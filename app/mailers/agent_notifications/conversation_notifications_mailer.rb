class AgentNotifications::ConversationNotificationsMailer < ApplicationMailer
  def conversation_creation(conversation, agent, _user)
    ...
    subject = I18n.t('email_subjects.conversation_created', name: @agent.available_name, id: @conversation.display_id, inbox: inbox_name)
  ...
  def conversation_assignment(conversation, agent, _user)
    ...
    @agent = agent
    @conversation = conversation
    subject = I18n.t('email_subjects.conversation_assigned', name: @agent.available_name, id: @conversation.display_id)
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
  ...
  def conversation_mention(conversation, agent, message)
    ...
    @conversation = conversation
    @message = message
    subject = I18n.t('email_subjects.conversation_mention', name: @agent.available_name, id: @conversation.display_id)
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
  ...
  def assigned_conversation_new_message(conversation, agent, message)
    ...
    @agent = agent
    @conversation = conversation
    subject = I18n.t('email_subjects.assigned_conversation_message', name: @agent.available_name, id: @conversation.display_id)
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
  ...
  def participating_conversation_new_message(conversation, agent, message)
    ...
    @agent = agent
    @conversation = conversation
    subject = I18n.t('email_subjects.participating_conversation_message', name: @agent.available_name, id: @conversation.display_id)
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
